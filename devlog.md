...
同时通过ncu-ui看了官方的kernel,是2阶段方案, flash_fwd_splitkv_kernel负责对固定的q & kv seq处理, flash_fwd_splitkv_combine_kernel负责合并, 同时全寄存器化, 通过精心设计的布局, 让在smem只存储原始q & k & v, 所有中间结果都寄存器化, 包括P @ V步骤中P的fp32到fp16转换也是在寄存器中进行. 下一步考虑按照该思路来实施. 这样的好处是对于短kv依旧是端到端, 对于长kv会自动分摊到多block, 最后在block归约,增大并行度,是用更多的global sector载入换取更高的计算单元使用率的做法. 下一步考虑按照该方案实现v8, 原定抽出qhead优先的逻辑来优化的方案放弃,该方案也许随着优化能从54toks/s上升到60-70, 但是随着长上下文,还是距离官方有明显差距,官方既然放弃了单kernel端到端,这里也尝试.

### 20260604
1. 增加q_seqlen() == 1的断言确认e2e可以工作, 进一步确认模型推理时q seqlen都是1逐次迭代的.
...
2. 只给sTensor_k增加Swizzle<3, 2, 3>，bank conflict有明显优化(c2acbdebf191cb121dac0d38cf7a556862b4319c)
...
2. out chunk归约后的last max和last sum写回有race，不能保证block夸多行就是安全的。
3. 感受到了由于编译器寄存器优化顺序带来的ulp级别差异，见commit `31f64ab`，完全一致的逻辑，仅仅是提出了一个计算步骤到变量，在端到端会产生不同输出，虽然都有合法语义。
