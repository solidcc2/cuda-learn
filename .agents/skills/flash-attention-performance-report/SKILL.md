...
- Treat `analysis/artifacts/e2e/perf_eval_results.json` as a measurement artifact, not as truth if the corresponding log shows an error.
...
- E2E parser: `flash_attention_backend/bench/e2e/parse_perf_eval_logs.py`
- Structured e2e result: `flash_attention_backend/analysis/artifacts/e2e/perf_eval_results.json`
...
- Optional NCU collector: `flash_attention_backend/analysis/run_ncu_case.sh`
- Optional NCU summary builder: `flash_attention_backend/analysis/summarize_ncu_report.py`
...
当用户只说“更新报告”或“刷新报告”，但没有说明是否重跑 benchmark 时，优先按下面的中文选项提示用户选择一次：

1. `仅更新报告（推荐）`
   - 使用已有 `report_inputs.json`
   - 不重跑任何 bench
2. `刷新轻量数据并更新报告`
   - 运行 `analysis/run_perf_eval.sh`
3. `刷新全量数据并更新报告`
   - 运行 `analysis/run_perf_eval.sh full`
4. `刷新轻量数据并重跑 NCU`
   - 运行 `analysis/run_perf_eval.sh --with-ncu --ncu-case <case>`
5. `刷新全量数据并重跑 NCU`
   - 运行 `analysis/run_perf_eval.sh full --with-ncu --ncu-case <case>`

提示要求：

- 选项说明必须使用中文
- 默认推荐项始终是 `仅更新报告（推荐）`
- 只有用户明确选择需要重跑时，才进入对应 bench / NCU 流程
- 如果用户选择 NCU 选项，但没给 case，继续用中文追问支持的 case
...
- `--with-ncu`: explicitly enables NCU collection for a supported profiling case
...
The analysis shell script is the unified collection entrypoint and regenerates `analysis/artifacts/e2e/perf_eval_results.json` together with structured artifacts under `analysis/artifacts/`.

当用户明确要求运行 benchmark 时：

- `轻量模式`：优先 smoke / 快速刷新
- `全量模式`：用于正式刷新报告数据
- 如果用户明确要求刷新 profiling / NCU，才可附加 `--with-ncu`
...
- Discover measured cases from `analysis/artifacts/e2e/perf_eval_results.json` and validate suspicious rows against `analysis/artifacts/e2e/perf_logs/*.log`.
...
If `analysis/artifacts/report/report_inputs.json` exists, prefer it as the canonical report input and fall back to `analysis/artifacts/e2e/perf_eval_results.json` for e2e-only reports.

If `report_inputs.json` contains a populated `profiling` section, use it as the canonical profiling input. Otherwise mark profiling as `未采集`.
...
- 当前 `analysis/run_perf_eval.sh` 可以统一采集 `e2e`、`op`、`Correctness` 和 `版本实现与优化摘要`；`Profiling/NCU` 仅在显式传入 `--with-ncu` 时采集，缺失时应明确写 `未采集`。
