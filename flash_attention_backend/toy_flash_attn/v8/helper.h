...
__device__ __forceinline__ scalar_t check_nan_val(scalar_t x, const char* tag) {
#ifdef DEBUG_NUMERIC
    if (isnan(x)) {
#ifdef __CUDA_ARCH__
...
template<typename scalar_t>
__device__ __forceinline__ scalar_t check_non_finite_val(scalar_t x, const char* tag) {
#ifdef DEBUG_NUMERIC
    if (!isfinite(x)) {
#ifdef __CUDA_ARCH__
...
__host__ __device__ __forceinline__ uint32_t ceil_pow2_u32(uint32_t x) {
    if (x <= 1) return 1;
    --x;
    x |= x >> 1;
    x |= x >> 2;
...
__host__ __device__ __forceinline__ int64_t round_up(int64_t x, int64_t base) {
    return (x + base - 1) / base * base;
}

// online softmax sub 只需要处理-inf
template<typename scalar_t>
__device__ __forceinline__ scalar_t softmax_sub(scalar_t a, scalar_t b) { 
    return (isfinite(a) && isfinite(b)) ? a - b : scalar_t{-INFINITY};
}

// online softmax div 除0表示无效位
template<typename scalar_t>
__device__ __forceinline__ scalar_t softmax_div(scalar_t a, scalar_t b) {
    return (b != scalar_t(0)) ? a/b : scalar_t{0};
}

template<typename scalar_t>
__device__ __forceinline__ scalar_t valid_mul(scalar_t valid_a, scalar_t b) {
    return valid_a == scalar_t(0) ? scalar_t(0) : valid_a * b;
}

__device__ __forceinline__ void busy_wait(unsigned long long delay_cycles) {
