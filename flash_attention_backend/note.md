...
额外query/value相关输入，主要是某些特定FA3/FA4路径需要，普通场景不用。
...
logits soft cap。
大于0时会对attention logits做soft cap限制
**TODO** 什么东西？

- alibi_slopes

ALiBi偏置参数。可按head / bach*head提供。不是所有FA版本都支持。
**TODO** 涉及transformer的alibi bias,待确认
...
是否返回attention probabilites。测试用，推理不用
**TODO**是什么，原理呢？
...
输出Tensor,如果提供kernel会直接写到这里
**TODO** 这里的输出是输出token的hidden state？
...
**TODO** 具体是什么东西，和输入q怎样协作，prefix长度？
